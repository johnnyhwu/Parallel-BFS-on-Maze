...
struct threadArgs2
{
    ...
    pthread_barrier_t* barrier1;
    // spinning_barrier* barrier1;
    pthread_barrier_t* barrier2;
    // spinning_barrier* barrier2;
};
...
// Implemented by: https://stackoverflow.com/a/8120707/10522203
...
struct threadArgs4
{
    ...
    // pthread_barrier_t* barrier1;
    SpinningBarrier* barrier1;
    // pthread_barrier_t* barrier2;
    SpinningBarrier* barrier2;
};

// same as parallelBFS2; but barrier is implemented by spin lock
void CPUSolver::parallelBFS4(unsigned int thread_num) {
    ...
    SpinningBarrier barrier1(thread_count);
    // pthread_barrier_init(&barrier1, NULL, thread_count);
    SpinningBarrier barrier2(thread_count);
    // pthread_barrier_init(&barrier2, NULL, thread_count);


    threadArgs4 args_template;
    ...
    threadArgs4* args_arr = new threadArgs4 [thread_count];
    for(int i=0; i<thread_count; i++) {
        args_arr[i] = args_template;
        args_arr[i].id = i;
    }
    ...
    // create thread
    for(int i=0; i<thread_count; i++) {
        pthread_create(&thread_handles[i], NULL, parallelBFS4Fn, args_arr+i);
    }
...
void* CPUSolver::parallelBFS4Fn(void* argss) {
    
    threadArgs4* args = (threadArgs4*) argss;
